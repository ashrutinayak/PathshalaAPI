...
fclose($file);
	...
	include("includes/config.php");
		...
		{
			$query_module=$con->select_query("module_add","*","Where class_id='".$class_id."' and student_id='".$student_id."' and module_status=0");
			if($con->total_records($query_module) != 0)
			{
				...
				while($row_module = mysqli_fetch_array($query_module))
				{
					...
					header('Content-type: application/json');
					echo json_encode(array("status"=>1,"moduledetail"=>$ModuleList,"message"=>"Pending module Listed Successfully."));
				}
				else
				{
					header('Content-type: application/json');
              		echo json_encode(array("status"=>0,"message"=>"No module are incompelted."));
				}
